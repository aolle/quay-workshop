...
Some of the OCI artifacts that we are going to use on the next sections are not allowed by default. For example, the Helm charts and signed images are allowed by default, but the encrypted container images not (at least at the time of this writing). Follow this step if you want to upload encrypted images to Quay.

As a previous step, we are going to configure the `ALLOWED_OCI_ARTIFACT_TYPES` property for allowing some of the OCI artifacts that we will use soon.

* Open a browser window and log in to the OpenShift Container Platform web console.

* Click `Operators` → `Installed Operators`.

* Select `quay-workshop` as the project and click `Quay Registry`. Click our `registry`.
...
* Open the Quay Dashboard and open the `olleb/helm/quarkus` repository and go to `Tags`.
...
====
This section shows how we can sign and store the result artifacts in our Quay registry, and how we can use Quay for storing OCI artifacts like signatures.
This is NOT a security guide neither a registry hardening guide.
We should decide when the source registry is trusted or not; if signing an image after uploading it into the registry is enough for us (or we prefer sign it before pushing); etc.
====
...
NOTE: strictly speaking, signing the image referencing it using a tag is not safe; it can lead us to sign a different image than the intended one. The correct way to do it would be using the image digest `@sha256:...`. Those steps will be updated in the future before `cosign` drops that functionality.
