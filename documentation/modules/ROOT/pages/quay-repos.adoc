...
* Click on the `SHA256` value under `MANIFEST` section of the `3.1.0` tag to view the layers dashboard.
...
* Select `Docker Pull (by tag)` from the `Image Format` drop-down menu and click on the `Copy Command` button.

image::repositories/fetchtag.png[]

* Switch to a terminal, paste the command you copied from the Quay UI, and then execute it. Make sure you have Docker or podman installed on your system.. If you are using podman, replace `docker` by `podman` before execute the command.
Since we created the repository as `Public`, no login is required to pull the image.
...
* Click on `Restore Tag` button. This will restore the image for that tag to the previous image.
