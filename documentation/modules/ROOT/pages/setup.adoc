...
OpenShift Data Foundation (ODF) offers multiple deployment models depending on the storage capabilities required by the workload. In this guide, we focus on
the *Multicloud Object Gateway* (MCG) mode, which provides a lightweight and S3-centric object storage service.
