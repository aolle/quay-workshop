...
IMPORTANT: If the init container fails when deploying the Quay registry, check if a LimitRange exists in the OCP project. The RHPDS environment creates one automatically.

[#exportenvvar]
== Export environment variables

The command-line or shell will be used during the workshop. The `QUAY_HOSTNAME` environment variable has to be set in the working terminal before executing the commands. Alternatively, we can provide the hostname directly.
